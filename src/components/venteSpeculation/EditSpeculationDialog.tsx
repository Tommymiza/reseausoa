...
interface EditSpeculationDialogProps {
  open: boolean;
  speculationId: number | null;
  onClose: () => void;
  onSpeculationUpdated: (speculationId: number) => void;
}

export default function EditSpeculationDialog({
  open,
  speculationId,
  onClose,
  onSpeculationUpdated,
}: EditSpeculationDialogProps) {
  const { updateSpeculation, getSpeculation, getSpeculations } =
    speculationStore();
  ...
  useEffect(() => {
    if (open && speculationId) {
      getSpeculation({ id: speculationId })
        .then((speculation) => {
          reset({
            nom: speculation.nom,
        ...
        .catch((error: unknown) => {
          ...
          toast.error(message ?? "Impossible de récupérer la spéculation");
        });
    }
  }, [open, speculationId, getSpeculation, reset]);

  const onSubmit = async (data: { nom: string; type: string }) => {
    if (!speculationId) return;
    ...
    try {
      ...
      onSpeculationUpdated(speculationId);
      reset();
      onClose();
    } catch (error: unknown) {
      const message = error instanceof Error ? error.message : undefined;
  ...
  return (
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <form onSubmit={handleSubmit(onSubmit)}>
        <DialogTitle>Modifier la spéculation</DialogTitle>
        <DialogContent>
          <TextField
            {...register("nom")}
